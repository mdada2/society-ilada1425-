 ...
 */
export const exportCSV = async (headers: string[], rows: any[][], filename: string) => {
    try {
        ...
        const dataLines = rows.map(row => row.map(formatCell).join(',')).join('\n');
        ...
        console.log('📊 Exporting CSV:', safeFilename);
...
/**
 * Export data as TSV with UTF-8 BOM for Excel compatibility
 */
export const exportTSV = async (headers: string[], rows: any[][], filename: string) => {
    try {
        ...
        const safeFilename = filename.endsWith('.csv') ? filename : `${filename}.csv`;
        console.log('📊 Exporting TSV:', safeFilename);
        await downloadBlob(blob, safeFilename);
    } catch (error) {
